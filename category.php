  ...
  <div class="container">
    <div class="row">
      <div
        ...
        <h1 class="h2 mb-4">Showing items from <mark><?php $_GET['cat'] ?></mark></h1>
        <?php
    if(isset($_GET['cat'])){
        $cat_key = trim($_GET['cat']);
        $get_post = mysqli_query($connect,"SELECT * FROM post WHERE category = '{$cat_key}'  ORDER BY date_created DESC");
        if(!$get_post){
            echo "Error Occured";
        }else{
